...
public class MecanumChassis {
    ...
    public boolean moveTo(double targetX, double targetY, double maxSpeed) {
        ...
        if (!atXTarget) {
            ...
            xPower = Math.sqrt(Math.abs(xDiff)) * xDiff < 0 ? 5.0 : -5.0;
        }
        if (!atYTarget) {
            yPower = Math.sqrt(Math.abs(yDiff)) * yDiff < 0 ? 5.0 : -5.0;
        }

        moveBy(xPower, yPower, angleTarget, maxSpeed);
    ...
    public boolean turnTo(double angleTarget, double maxSpeed, Side side) {
        ...
        boolean atTarget = Math.abs(angleTarget - currentAngle()) <= angleTolerance;
    ...
    public void runQueue() {
        ...
        if (queue.get(0).run()) { queue.remove(0); }
    }
     ...
     */
    private class MoveToQueueable implements Queueable {
     ...
     */
    private class TurnToQueueable implements Queueable {
